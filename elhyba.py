...
ownerID = int("6515320646") #ايدي الادمن 
...
async def generator_and_about(app,m):
    ...
    if m.text == "بـايـروجـرام":
        ...
        try:
          ...
          return await code_ask.reply("رمز الهاتف غير صالح! \n/start", quote=True)
        try:
          await c.sign_in(phone, hash, code)
        except:
          pass
...
app.start()
bot.start()
print("تم تشغيل البوت عمك الديلر يوثخ ")
idle()
